...
# Custom functions (copy and paste in your code)
source('RSQLiteGetTag.R')
source('RSQLiteGetTagTime.R')
source('RSQLiteGetTable.R')
...
# Read entire database
SQLDATA <- RSQLiteGetTable(sqlfile)
...
for(tag_number in 1:length(TagList))
{
  ...
  TAGDATA <- RSQLiteGetTag(sqlfile,TagList[tag_number])
  ...
  TIMEDATA = RSQLiteGetTagTime(sqlfile,TagList[tag_number],start_time,end_time)
}